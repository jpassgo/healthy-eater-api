...
public class AuthenticationController {
  ...
  public ResponseEntity authenticate(
    ...
    Optional<UserAccount> optionalUserAccount = userAccountRepository.findByUserCredentialsUserName(userCredentials.getUsername());
  ...
  private boolean doesPasswordMatch(@RequestBody UserCredentials userCredentials, Optional<UserAccount> optionalUserAccount) {
    BCryptPasswordEncoder encoder = new BCryptPasswordEncoder(10, new SecureRandom());
    return encoder.matches(userCredentials.getPassword(), optionalUserAccount.get().getUserCredentials().getPassword());
  }
}
